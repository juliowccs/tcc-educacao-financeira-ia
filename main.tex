...
\section{DESENVOLVIMENTO}
  ...
  \subsection{MAPEAMENTO DA BASE DE CONHECIMENTO FINANCEIRO}

    Para capacitar o sistema a fornecer respostas confiáveis e contextualizadas, realizou-se o mapeamento dos principais conceitos de finanças pessoais, gestão orçamentária e economia comportamental. O núcleo consultivo da base foi estruturado a partir dos seguintes documentos: o Caderno de Educação Financeira do Banco Central do Brasil, a coleção TOP de Planejamento Financeiro Pessoal, o Caderno CVM Volume 1: Vieses do Investidor e as diretrizes da Estratégia Nacional de Educação Financeira (ENEF).

    A transformação desse material normativo e extenso em artefatos indexáveis demandou um \textit{pipeline} estruturado de pré-processamento, particionamento semântico e enriquecimento de metadados. A primeira etapa consistiu na limpeza dos documentos, removendo elementos que não agregam valor semântico, como capas, sumários, rodapés, exercícios e créditos.

    Em seguida, em vez de um extrator de PDF tradicional, que tende a desconstruir a ordem lógica dos documentos, foi utilizado o \textit{Docling}. A biblioteca utiliza modelos de visão computacional voltados ao reconhecimento avançado de \textit{page layout} e estruturas tabulares, preservando a coesão semântica de parágrafos, cabeçalhos e tabelas.

    O particionamento foi executado com o mecanismo \textit{HybridChunker}, calibrado para um limite de 450 \textit{tokens}, dimensionamento compatível com a janela do modelo de vetorização \textit{sentence-transformers/paraphrase-multilingual-mpnet-base-v2}. Para evitar que os blocos ficassem curtos demais ou incompletos, configurou-se o agrupamento de trechos vizinhos e a repetição dos cabeçalhos em tabelas que foram divididas.

    Por fim, cada pedaço de texto recebeu metadados essenciais: um identificador único, o documento de origem, as páginas correspondentes, a seção e o assunto principal. Dessa forma, além de encontrar o conteúdo pelo contexto, a IA consegue indicar com precisão a fonte e a página consultada, o que ajuda a evitar respostas inventadas ou imprecisas.

    O resultado desta etapa consiste em um arquivo consolidado contendo os fragmentos estruturados, conforme ilustrado na Figura~\ref{fig:exemplo_chunk}. Essa padronização permite tanto a busca semântica pelo conteúdo textual quanto a extração direta dos metadados para citação e auditoria pelo sistema.

    \begin{figure}[ht]
    \centering
    \includegraphics[width=0.75\textwidth]{caderno_cidadania_financeira_tratado_c0079.png}
    \caption{Exemplo da estrutura JSON de um fragmento indexável gerado no pipeline.}
    \label{fig:exemplo_chunk}
    \end{figure}

    A partir dessa organização, cada partição preserva seu identificador único e a hierarquia temática do documento original. Com isso, o pipeline garante a integridade das referências normativas durante as consultas realizadas pela inteligência artificial.

  \subsection{ARQUITETURA E DESENVOLVIMENTO DA APLICAÇÃO WEB}
